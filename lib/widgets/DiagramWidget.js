...
export class DiagramWidget extends React.Component {
  ...
  onMouseMove(event) {
    ...
    if (action instanceof SelectingAction && multiselectDrag) {
      ...
      action.selectionModels.forEach(model => {
        ...
        //Applied a discrete function to node movement
        if(model.model instanceof NodeModel){
          model.model.x = this.discreteFuntion(model.model.x,30);
          model.model.y = this.discreteFuntion(model.model.y,30);
        }
      });
  ...
  discreteFuntion(n,scale){
    let floorValue = Math.trunc(n/scale) * scale;
    let ceilValue = floorValue + scale;
    return (n - floorValue > ceilValue - n ? ceilValue : floorValue);
  }
